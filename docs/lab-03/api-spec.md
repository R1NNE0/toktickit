...
| POST `/auth/login` | Anonymous/bootstrap or unrestricted existing session plus Origin/CSRF; `{ email, password }`; restricted must-change sessions cannot re-login | 200 `{ user: CurrentUser, csrfToken }`; new cookie/session replaces previous browser session; initial password yields restricted authenticated session | 403 PASSWORD_CHANGE_REQUIRED for an existing restricted session, even with valid credentials; no session replacement or account switch. Otherwise 400 malformed email/missing/oversized fields; same 401 message “Unable to sign in. Check your credentials or contact your administrator.” for unknown/wrong/inactive; 429 throttled |
...
`CurrentUser = { id, name, email, role, isActive, mustChangePassword }`. It never includes passwordHash/emailNormalized/session records. Initial-password login establishes a restricted authenticated session. While it requires a password change, only GET `/auth/csrf`, GET `/auth/me`, POST `/auth/change-password` and POST `/auth/logout` are allowed session operations. **Every normal API and POST `/auth/login` through that restricted session returns 403 PASSWORD_CHANGE_REQUIRED.** Apply this gate before login credential evaluation/session replacement; even valid credentials for the same or another account cannot re-login through it. Rejected re-login leaves the restricted identity and session unchanged. Allowed mutations still require Origin/CSRF checks. Successful password change rotates into a normal session; logout revokes the restricted session, after which a fresh anonymous CSRF bootstrap/login flow is possible. Frontend bootstraps with me/csrf before entering a protected screen; it must not infer auth from localStorage.
