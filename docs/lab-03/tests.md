...
| API-02 | API | FR-02; BR-02, BR-08; AC-02 | Initial-password restricted login, invalid/current/reused/mismatched new passwords, successful change | Only auth allowlist accessible until success; old password/session fails; new normal session works | `server/tests/lab-03/auth.api.test.ts` | Planned / Not Run |
...
| API-05 | Security | FR-04; BR-01, BR-02, BR-10, BR-28; AC-02, AC-03, AC-05 | Every protected endpoint with no session, must-change session, inactive user and each role | 401/403 as matrix; fresh role/activation state enforced | `server/tests/lab-03/authorization.api.test.ts` | Planned / Not Run |
...
| UI-06 | Component/regression | FR-07, FR-12, FR-14; BR-04, BR-19, BR-21, BR-22; AC-08, AC-13, AC-15 | Detail upload/download/remove, Public Comments and resolution indication; no Internal Notes | Correct controls/counts/removal states, limits and no formal status action | `client/tests/lab-03/RequesterRegression.test.tsx` | Planned / Not Run |
...
|---|---|---|---|---|
| P1 | Sprint 3 engineering contract and test plan: these six documents, approved decisions and traceability | Engineering decisions approved in Phases 2.1/2.2; later implementation authorization | AC-26; EVID-01 | `docs/lab3-engineering-contract` |
